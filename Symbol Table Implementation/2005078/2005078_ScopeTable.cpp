...
std::string ScopeTable::insert(const std::string symbolName, const std::string symbolType)
{
    if (lookup_check(symbolName))
    {
        std::string ans= "     '" + symbolName + "' already exists in the current ScopeTable# " + getUniqueId();
        return ans;
    }
